...
app.include_router(api_router, prefix="/api"). app/api.py.    from fastapi import APIRouter, Depends, HTTPException
...
@router.get("/health")
def health():
    return {"status": "ok"}.   app/db.py.  from sqlmodel import SQLModel, create_engine, Session
...
def get_session():
    with Session(engine) as session:
        yield session.  b. app/models.py. from sqlmodel import SQLModel, Field
...
def authenticate(email: str, password: str, session: Session):
    ...
    return user.  app/predictor.py.   import random

def predict_income():
    base = random.randint(800, 1500)
    trend = random.uniform(0.9, 1.2)
    return round(base * trend, 2).  .   from fastapi import FastAPI
from app.api import router as api_router
from app.db import create_db
...
@router.get("/health")
def health():
    return {"status": "ok"}.    from sqlmodel import SQLModel, create_engine, Session
